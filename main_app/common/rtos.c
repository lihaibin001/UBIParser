...
static xQueueHandle pxTaskQueue[OS_NUM_TASKS];//message queue handle
static xTaskHandle  pxTaskHandle[OS_NUM_TASKS];//task handle
static xSemaphoreHandle  pxResources[OS_NUM_RESOURCES];//Counting Semaphore for system resource
...
Task_State_Type OS_Task_State(Task_Type task_id)
{
    ...
    {
        if(TRUE == xTaskIsTaskSuspended(pxTaskHandle[task_id]))
        {
            return(SUSPENDED);
        }
        else
        {
            return(INVALID_STATE);/*reserved for other state*/
        }
    }
    else
    {
...
Status_Type OS_Activate_Task(Task_Type task_id)
{
    ...
    {
        return (E_OS_ID);         /* Invalid task number */
    }
}
/*===========================================================================*\
...
static void OS_Create_Task(const OS_TCB_Init_Type * task_info, xQueueHandle *pQuehandle,xTaskHandle *pTaskhandle)
{
    ...
    *pQuehandle = xQueue;
    ...
    xTaskCreate( (pdTASK_CODE)(task_info->Entry) \
   	            , (const signed char *)task_info->name \
...
Status_Type OS_Send_Message(Task_Type task_id, Message_Type msg)
{
    ...
    {
        return (E_ERROR);       /* Queue full */
    }
    else
    {
        return (E_OK);     /* else return there was no message */
    }
}

/*===========================================================================*\
 *    Function: OS_Send_Message_FromISR
 *
 *  Parameters: 'task_id' is task to send message to
 *              'msg'   pointer value to write to mailbox
 *
 ...
 *
\*===========================================================================*/
Status_Type OS_Send_Message_FromISR(Task_Type task_id, Message_Type msg)
{
    Message_Type xmsg = msg;
...
Status_Type OS_Receive_Message(Task_Type task_id,Message_Type * msg)
{
    ...
    {
        return (E_ERROR);     /* else return there was no message */
    }
    else
    {
        return (E_OK);     /*OK */
...
static Status_Type os_get_resource(Resource_Type resid)
{
    ...
    {
        return (E_OS_RESOURCE);/* default is resource not available */
    }
}
...
Status_Type OS_Wait_Resource(Resource_Type resid, Tick_Type timeout)
{
    ...
    {
       return (E_TIMEOUT);/* semaphore wait timer out */
    }
}
...
{
    int32_t  HigherPriorityTaskWoken = false; //fixme.flag indicates if higher priority task woken.
    ...
    if (NULL  != pxResources[resid])
    {
        if(pdTRUE == xSemaphoreGiveFromISR(pxResources[resid], &HigherPriorityTaskWoken))
        {
            ...
            return (E_OK);                                                                                             //would switch in rtc interrupt.
        }
        else
        {
            return (E_ERROR);/*error*/
...
/*===========================================================================*\
 *    Function: vApplicationStartHook
 *
 *  Parameters: None
 *
 *     Returns: None
...
extern void vApplicationStartHook(void)
{
    ...
    ;
#endif
//    RL_Finish_Awake_Sequence();       // 
    mn_Initialize();                             /* call initialization routines after power on */
    Disable_Interrupts();
