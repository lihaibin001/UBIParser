...
static void sFLASH_LowLevel_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(sFLASH_CS_GPIO_CLK | sFLASH_SPI_MOSI_GPIO_CLK | sFLASH_SPI_MISO_GPIO_CLK |
                         sFLASH_SPI_SCK_GPIO_CLK | RCC_APB2Periph_GPIOC, ENABLE);
  ...
  GPIO_InitStructure.GPIO_Pin = sFLASH_HOLD_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(sFLASH_HOLD_PORT, &GPIO_InitStructure);

  GPIO_InitStructure.GPIO_Pin = sFLASH_WP_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(sFLASH_WP_PORT, &GPIO_InitStructure);

  GPIO_SetBits(sFLASH_HOLD_PORT, sFLASH_HOLD_PIN);
  GPIO_SetBits(sFLASH_WP_PORT, sFLASH_WP_PIN);
}
