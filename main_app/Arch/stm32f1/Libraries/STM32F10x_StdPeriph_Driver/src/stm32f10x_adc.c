...
/** @defgroup ADC_Private_Variables
  * @{
  */
//static bool ad_initialized = false;
static bool ad_1_initialized = false;
static bool ad_9_initialized = false;
static bool ad_13_initialized = false;
//static uint16_t ADC_ConvertedValue;
static uint16_t adc_initialized_channel=ADC_Channel_0;
...
/** @defgroup ADC_Private_Functions
  ...
  */
/**********************************************************************
 * Description: Initialize A/D operation
 *  Parameters: 
 *     Returns: 16 bit A/D count 
 *********************************************************************/
static void ad_1_initialize(void)
{
...
//	ADC_InitStructure.ADC_NbrOfChannel      =1;       //Scan channel number
//	ADC_Init(ADC1,&ADC_InitStructure);
//	// config the ADC clock
...
//	while(ADC_GetCalibrationStatus(ADC1));
	...
	ad_1_initialized = true;
    adc_initialized_channel=ADC_Channel_1;
}
 ...
 *     Returns: 16 bit A/D count 
 *********************************************************************/
static void ad_9_initialize(void)
{
	ADC_InitTypeDef  ADC_InitStructure;
	GPIO_InitTypeDef GPIO_InitStructure;
//	DMA_InitTypeDef DMA_InitStructure;
 ...
 *     Returns: 16 bit A/D count 
 *********************************************************************/
static void ad_13_initialize(void)
{
//	ADC_InitTypeDef  ADC_InitStructure;
//	GPIO_InitTypeDef GPIO_InitStructure;
////	DMA_InitTypeDef DMA_InitStructure;
...
//	while(ADC_GetCalibrationStatus(ADC1));
	...
	ad_13_initialized = true;
    adc_initialized_channel=ADC_Channel_13;
}

/**********************************************************************
 * Description: AD_Read 
 *  Parameters: Channel to read 
 *              pointer to 16 bit left justified count
 *     Returns: E_OK if conversion is successful
 *********************************************************************/
uint16_t AD_Read(uint_fast8_t channel)
{	
    uint16_t counts;

#if 0
    if (channel != adc_initialized_channel)
    {
        switch(channel)
        {
            case ADC_Channel_1:
    ...
    counts = ADC_GetConversionValue(ADC1);         /* read appropriate results   */
    return (counts);
#endif

#if 1
    if (channel==ADC_Channel_13)
    {
        if (!ad_13_initialized)
        {
            ad_13_initialize();
            uDelay(100);//delay 0.5ms
            ad_1_initialized = false;
        }
        counts = ADC_GetConversionValue(ADC1);         /* read appropriate results   */
        return (counts);
    }
...
/**
  * @brief  Initilize ADC1 common registers for channel1 and channel13.
  * @param  None.
  * @retval None
  */
