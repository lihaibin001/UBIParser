...
typedef struct AT_Cmd_Param_tag                           
{
    ...
    char APN_user[20];
    char APN_pwd[20];
    IP_Link_Setting_t IP_Open_Setting;
    uint8_t send_len;
    uint8_t mux_type;
    uint8_t fun_type;
...
typedef enum TCP_Conn_State_Tag
{
    ...
    NET_REGISTERED,// 5
    NET_GPRS_ATTACHED,// 6
    NET_APN_REGISTERED,// 7
    NET_GPRS_ACTIVATED, // 8
    NET_IP_GOT, // 9
    NET_TCP_CONNECTED,// 10
    NET_DATA_TRANS,// 11
    NET_STATE_NUM
}TCP_Conn_State_T;
...
extern void vATProt_Set_TCP_State(uint8_t state);
extern uint8_t vATProt_Get_TCP_State(void);
