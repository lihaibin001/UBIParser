...
typedef struct GSM_Info_tag
{
    ...
    char imsi[15];
    char imei[15];
    uint8_t tcp_state; //network state
    int8_t dev_temp;
...
static void prvATProt_Resp_Decode_IMSI_Query(void);
...
static uint8_t sms_rx_mode=SMS_IDLE; //0:not reading; 1:reading
...
static const AT_Cmd_Info_t AT_Cmd_Info_table[AT_CMD_NUM] =
{
    ...
    { 20, "AT+CIMI",        EXCUTE,   MSec_To_Ticks(1000),  TRUE},   /* AT_CMD_GET_IMSI_NO */
    ...
    { 1,  "",               EXCUTE,   MSec_To_Ticks(5000),  TRUE},  /* AT_CMD_SEND_POST_DATA */
    { 5,  "AT+CGACT=",      EXCUTE,   MSec_To_Ticks(2000),  TRUE},  /* AT_CMD_ACTIVE_PDP_CONTEXT */
    { 1,  "AT+CGATT=1",     EXCUTE,   MSec_To_Ticks(1000),  TRUE},  /* AT_CMD_CGATT */
    ...
    { 1,  "AT+CIPSTART=",   EXCUTE,   MSec_To_Ticks(20000), TRUE},  /* AT_CMD_IP_OPEN_EXCUTE */
    ...
    { 1,  "AT+QICLOSE",     EXCUTE,    MSec_To_Ticks(300),  TRUE},  /* AT_CMD_QICLOSE */
    ...
    { 1,  "AT+CIPSEND=",    EXCUTE|FATAL,   MSec_To_Ticks(16000),  TRUE},  /* AT_CMD_IP_SEND_DATA_EXCUTE */
    ...
    { 3,  "AT+QLTS",        QUERY,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_CCLK */
...
static AT_URC_decode_t AT_URC_decode_table[AT_URC_NUM] = 
{
   ...
   {AT_URC_SIGNAL, "+CSQ:", prvATProt_URC_decode_Signal, NULL},
   {AT_URC_RECV, "+CIPRXGET:", prvATProt_URC_decode_Recv, NULL},
   ...
   {AT_URC_CCLK, "+QLTS:", prvATProt_URC_decode_Clock, NULL},
};
...
static const AT_Resp_decode_t AT_Resp_decode_table[] = 
{
    ...
    {AT_CMD_GET_IMSI_NO,    prvATProt_Resp_Decode_IMSI_Query},
...
extern void vATProt_Power_Off(void)
{
    ...
    IO_GSM_PWR_ON_OUT(Bit_RESET);
    rl_delay_without_schedule(2000); // wait for logout net
...
*/
    ...
    if(AT_pending_tx_cmd == AT_CMD_NONE)
    {
        ...
        {
//            DEBUG_PRINT0(DEBUG_HIGH,"[2G] IPSEND,Receive >,but no data is send, something wrong!!!\n\r");
...
/*
    if (AT_pending_tx_cmd == AT_CMD_NONE)
    {
        if ((TCP_IDLE == prvATProt_Get_TCP_Mode()) && (1 == rx_pending_status))
        {
...
extern void vATProt_Check_Receive(void)
{
    ...
    {
        //Feed_Dog();
//        DEBUG_PRINT1(DEBUG_MEDIUM,"%c",data_byte);
...
char const * pcATProt_getImsiData(void)
{
    ...
    return "461234561231234";//Set 
    as 461234561231234 for test.
#else
...
/*            if (TCP_RXING == prvATProt_Get_TCP_Mode())
            {
                ...
                {
                    tcom_receiving_frame = false;
                    prvATProt_Decode_Ipdata(tcom_rx_buffer, tcp_rx_len);
                }
            }
            else */
            if(CR == data)
        ...
        case START_RX_DATA:
            if (rx_char_count == 8)
            {
                ...
                if (strncmp(tcom_rx_buffer,"+IPD",4)==0)
                {
                    for (i=0;i<9;i++)
                    {
                        if(*(tcom_rx_buffer+i)==':')
                    ...
                    // index 5 to ':', data length
                    for (i=5;i<tcp_rx_comma_pos;i++)
                    {
            ...
            {
                prvATProt_Rx_Buffer_Filling(data);
                if (rx_char_count < (tcp_rx_len+tcp_rx_comma_pos+1))
                {
                    AT_Resp_Rx_State = START_RX_DATA;
                    tcom_receiving_frame = true;
                }
                else
                {
                    AT_Resp_Rx_State = RX_IDLE;
                    tcom_receiving_frame = false;
                    prvATProt_Decode_Ipdata(tcom_rx_buffer+tcp_rx_comma_pos+1, tcp_rx_len);
                    prvATProt_Set_TCP_Mode(TCP_IDLE);
                }
            }
            else if ((CR == data) && tcom_receiving_frame)
            {
...
static void prvATProt_Resp_Common_Decode(char* tcom_rx_buffer, uint8_t rx_count)
{
...
#ifdef USE_DEBUG
    DEBUG_PRINT1(DEBUG_MEDIUM,"[2G rx]%s\n\r",tcom_rx_buffer);
#endif
    ...
    if(!prvATProt_URC_Decode(tcom_rx_buffer, rx_count))
    {
        ...
        if (AT_pending_tx_cmd == AT_CMD_IP_OPEN_EXCUTE)
        {   //For IP OPEN  error recovery
            if((!strncmp((char*)tcom_rx_buffer, CME_NW_OPENED_String, strlen(CME_NW_OPENED_String)))
                ||(!strncmp((char*)tcom_rx_buffer, CONNECT_OK_String, strlen(CONNECT_OK_String))))
            {
                at_cmd_res = AT_CMD_RES_OK; //set ok flag
                at_cmd_end = true;
                if (NET_TCP_CONNECTED>vATProt_Get_TCP_State())
                    vATProt_Set_TCP_State(NET_TCP_CONNECTED);
            }
            else if (!strncmp((char*)tcom_rx_buffer, CONNECT_FAIL_String, strlen(CONNECT_FAIL_String)))
            {
...
//                vATProt_Set_TCP_State(NET_GPRS_ACTIVATED);
        ...
        else if(!strncmp((char*)tcom_rx_buffer, DEACT_String, strlen(DEACT_String)))  //power down
        {
            ...
            prvATProt_Set_TCP_Mode(TCP_IDLE);
            sms_rx_mode=SMS_IDLE;
            DEBUG_PRINT0(DEBUG_HIGH,"[2G] PDP DEACT!\n\r");
        }
        ...
        else if(!strncmp((char*)tcom_rx_buffer, SHUT_OK_String, strlen(SHUT_OK_String)))
        {
            ...
            sms_rx_mode=SMS_IDLE;
            vATProt_Com_Reset();
            DEBUG_PRINT0(DEBUG_HIGH,"[2G] SHUT OK!\n\r");
        }
        ...
        else if(!strncmp((char*)tcom_rx_buffer, ERROR_String, strlen(ERROR_String)))
        {
            ...
            at_cmd_end = true;
            DEBUG_PRINT0(DEBUG_MEDIUM,"[2G] ERROR!\n\r");
...
#endif
		else if (!strncmp((char*)tcom_rx_buffer, SEND_OK_String, strlen(SEND_OK_String)))
		{
			at_cmd_res = AT_CMD_RES_OK;	//set ok flag
			at_cmd_end = true;
		...
		else if (!strncmp((char*)tcom_rx_buffer, REMOTE_CLOSE_String, strlen(REMOTE_CLOSE_String)))
		{
			at_cmd_res = AT_CMD_RES_OK;	//set ok flag
			at_cmd_end = true;
		...
		else if (!strncmp((char*)tcom_rx_buffer, OK_String, strlen(OK_String)))
		{
			...
			at_cmd_res = AT_CMD_RES_OK;
			at_cmd_end = true;
            if (NET_POWER_ON == vATProt_Get_TCP_State())
            {
...
#if 0
        else if (!strncmp((char*)tcom_rx_buffer, ModuleReady_String, strlen(ModuleReady_String)))
        {
            vATProt_Set_TCP_State(NET_POWER_ON);
        }
#endif
...
static void prvATProt_URC_decode_Recv(char* rx_buffer, uint8_t rx_count)
{
    ...
    switch(mode)
    {
        case 1:
            /* Data received */
//            if (TCP_IDLE == prvATProt_Get_TCP_Mode())
            {
                vATProt_Set_Rxtype(2);
//                vATProt_sendAT_Command(AT_CMD_IP_RX_MANUALLY_QUERY, NULL, NULL);
                DEBUG_PRINT0(DEBUG_MEDIUM,"[2G]: RX status 1!\n\r");
                rx_pending_status = 1;
            }
            break;
        case 2:
            /* Parse data */
            memset(IPDATA, 0x00, TCOM_RX_BUF_SIZE);
            IPDataLen = min(ATProt_decode_rx(rx_buffer),TCOM_RX_BUF_SIZE);
            rx_pending_status = 0;
            if(IPDataLen > 0)
            {
                DEBUG_PRINT1(DEBUG_MEDIUM,"[2G]:ACK received [%d]!\n\r",IPDataLen);
                if (ack_from_server == 0)
                    ack_from_server = 1;
                prvATProt_Set_Recv_Len(IPDataLen);
//                prvATProt_Set_TCP_Mode(TCP_RXING);
            }
            else if (IPDataLen == 0)
            {
...
static void prvATProt_URC_decode_Gprs(char* rx_buffer, uint8_t rx_count)
{
    ...
    if (state == 1)
    {
        ...
        if (NET_GPRS_ATTACHED == vATProt_Get_TCP_State())
            vATProt_Set_TCP_State(NET_APN_REGISTERED);
...
static void prvATProt_URC_decode_Sim(char* rx_buffer, uint8_t rx_count)
{
    ...
    {
        /* SIM OK */
        if (NET_ECHO_CLOSED == vATProt_Get_TCP_State())
            vATProt_Set_TCP_State(NET_SIM_CONNECTED);
        AT_Cmd_Param.sim_ready=1;
    }
    else
    {
        DEBUG_PRINT0(DEBUG_HIGH,"[2G]:SIM Not Ready!\n\r");
        /* SIM not OK */
...
//        if (NET_IMSI_GOT == vATProt_Get_TCP_State())
        uint8_t len=0;
        vATProt_Set_TCP_State(NET_REGISTERED);
        p_temp=Get_Filed_Data_Pointer(buf_ptr,2);
...
//            uint32_t lac=StrtoHex(p_temp+1,len-2);
        ...
        p_temp=Get_Filed_Data_Pointer(buf_ptr,3);
        len = Get_Next_Data_Len_Before_Comma(p_temp);
        if (len>0)
        {
    ...
    {
        /* error */
        DEBUG_PRINT1(DEBUG_MEDIUM,"[2G]:Wrong CREG [%d]!\n\r",netsts);
    }
    AT_Cmd_Param.netreg=netsts;
}
...
static void prvATProt_URC_decode_Temp(char* rx_buffer, uint8_t rx_count)
{
    ...
    {
        data = rx_buffer[10] - '0';
        if ((rx_buffer[11] != '.') && (rx_buffer[11] != '\n'))
        {
            data *= 10;
            data += rx_buffer[11] - '0';
        }
        net_Info.dev_temp = 0-data;
    }
    else
    {
        data = rx_buffer[9] - '0';
        if ((rx_buffer[10] != '.') && (rx_buffer[10] != '\n'))
        {
            data *= 10;
            data += rx_buffer[10] - '0';
        }
        net_Info.dev_temp = data;
    }
    DEBUG_PRINT1(DEBUG_MEDIUM,"[2G]: temperature [%d]\n\r", net_Info.dev_temp);
}
...
static void prvATProt_URC_decode_Clock(char* rx_buffer, uint8_t rx_count)
{
    ...
    clock_ref.year[0]='2';
    clock_ref.year[1]='0';
    clock_ref.year[2]=*(rx_buffer+8);
    clock_ref.year[3]=*(rx_buffer+9);
    clock_ref.month[0]=*(rx_buffer+11);
    clock_ref.month[1]=*(rx_buffer+12);
    clock_ref.day[0]=*(rx_buffer+14);
    clock_ref.day[1]=*(rx_buffer+15);
    clock_ref.hour[0]=*(rx_buffer+17);
    clock_ref.hour[1]=*(rx_buffer+18);
    clock_ref.min[0]=*(rx_buffer+20);
    clock_ref.min[1]=*(rx_buffer+21);
    clock_ref.sec[0]=*(rx_buffer+23);
    clock_ref.sec[1]=*(rx_buffer+24);
}
...
static void prvATProt_parse_sms(char* rx_buffer, uint8_t rx_count)
{
    ...
    {
        tmp_data[i]=StrtoHex(rx_buffer+(i*2));
        data_len++;
    }
...
static void prvATProt_Resp_Decode_SET_Echo(void)
{
    if (NET_WAIT_AT == vATProt_Get_TCP_State())
        vATProt_Set_TCP_State(NET_POWER_ON);
}
...
static void prvATProt_Resp_Decode_Answer_Call(void)
{
    DEBUG_PRINT0(DEBUG_LOW,"[2G]:Call coming, answered");
}
/*********************************************************
...
static void prvATProt_Resp_Decode_IMSI_Query(void)
{
    DEBUG_PRINT1(DEBUG_MEDIUM,"IMSI:%s\n\r",AT_Resp_data.Resp_IT);
    if ((AT_Resp_data.Resp_IT[0] >= '0') && (AT_Resp_data.Resp_IT[0] <= '9'))
    {
        memcpy(net_Info.imsi, AT_Resp_data.Resp_IT, sizeof(net_Info.imsi));   
//        vATProt_Set_TCP_State(NET_IMSI_GOT);
    }
}
/*********************************************************
...
static void prvATProt_Resp_Decode_IMEI_Query(void)
{
    ...
    imei_tmp[15]=0;
    DEBUG_PRINT1(DEBUG_MEDIUM,"IMEI:%s\n\r",net_Info.imei);
    imei_tmp[15]='f';
    for(i=0;i<8;i++)
    {
...
static void prvATProt_Resp_Decode_IP_Query(void)
{
    if (NET_GPRS_ACTIVATED == vATProt_Get_TCP_State())
    {
        vATProt_Set_TCP_State(NET_IP_GOT);
        //at_cmd_res = AT_CMD_RES_OK; 
        DEBUG_PRINT1(DEBUG_MEDIUM,"%s\n\r",(char *)AT_Resp_data.Resp_IT);
    }
}
...
#ifdef USE_DEBUG
    DEBUG_PRINT1(DEBUG_MEDIUM,"[2F TX]%s\n",tcom_tx_buffer);
#endif
//    prvATProt_Uart_Transmit(tcom_tx_buffer, tcom_tx_num_bytes);
    
    if(0==UART_DMA_tx(tcom_tx_buffer, tcom_tx_num_bytes))
    {
        DEBUG_PRINT1(DEBUG_MEDIUM,"[2F TX]DMA TX ERROR:%d,0\n",tcom_tx_num_bytes);
    }
...
static void prvATProt_Retransmit(void)
{
    ...
    if(0==UART_DMA_tx(tcom_tx_buffer, tcom_tx_num_bytes))
    {
        DEBUG_PRINT1(DEBUG_MEDIUM,"[2F TX]DMA TX ERROR:%d,1\n",tcom_tx_num_bytes);
    }
}
...
static void prvATProt_Wait_Resp(void)
{
    ...
    if( (OS_Time() > AT_Resp_Timeout) ||(at_cmd_res == AT_CMD_RES_ERR))//||(at_cmd_res == AT_CMD_RES_CME_ERR) )
    {
        ...
        if(tcom_frame_tx_attempts < AT_Tx_ATTEMPS)
        {
...
//               prvATProt_Set_TCP_Mode(TCP_IDLE);
               ...
               TelmApp_Set_Server_Ack_State(false);
        }
        else
        {
            ...
            {//Fatal command, reset module if failed.
                ...
                OS_Send_Message(OS_IOT_TASK,Build_Message(TM_EVT_RESET_MODULE, 0));
            }
...
//    else if(AT_pending_tx_cmd == AT_CMD_SEND_POST_DATA)
//    {
//        if((TCP_TX_POSTDATA == prvATProt_Get_TCP_Mode()) && (at_cmd_res == AT_CMD_RES_OK))
//        {
//            prvATProt_AT_Transmit_Reset();
//        }
//    }
...
static void prvATProt_AT_Command_Deep_encode_SendPostdata(void)
{
    ...
    else//Send Command
    {
        ...
        else
        {
            ...
            tcom_tx_num_bytes++;
            DEBUG_PRINT0(DEBUG_HIGH,"[2G]:ERROR:Tx Buffer Overflow!!\n\r");        
        }
#ifdef TEST_ACK_TIMER
...
{
    uint8_t Cmd_Data_length;
    strncpy(AT_Cmd_Param.IP_Open_Setting.IP_LinkType,"TCP",4);

    Get_IP_config(AT_Cmd_Param.IP_Open_Setting.Dest_IP, AT_Cmd_Param.IP_Open_Setting.Dest_port);

    // Copy link type
    tcom_tx_buffer[tcom_tx_num_bytes++] = CHAR_QUOTES;
    Cmd_Data_length = strlen(AT_Cmd_Param.IP_Open_Setting.IP_LinkType);
    strncpy(tcom_tx_buffer+tcom_tx_num_bytes, AT_Cmd_Param.IP_Open_Setting.IP_LinkType,Cmd_Data_length);
    tcom_tx_num_bytes = tcom_tx_num_bytes+Cmd_Data_length;
    tcom_tx_buffer[tcom_tx_num_bytes++] = CHAR_QUOTES;
    ...
    // Copy dest IP
    tcom_tx_buffer[tcom_tx_num_bytes++] = CHAR_QUOTES;
    Cmd_Data_length = strlen(AT_Cmd_Param.IP_Open_Setting.Dest_IP);
    strncpy(tcom_tx_buffer+tcom_tx_num_bytes, AT_Cmd_Param.IP_Open_Setting.Dest_IP,Cmd_Data_length);
    tcom_tx_num_bytes = tcom_tx_num_bytes+Cmd_Data_length;
    tcom_tx_buffer[tcom_tx_num_bytes++] = CHAR_QUOTES;
    ...
    // Copy dest port
    tcom_tx_buffer[tcom_tx_num_bytes++] = CHAR_QUOTES;
    Cmd_Data_length = strlen(AT_Cmd_Param.IP_Open_Setting.Dest_port);
    strncpy(tcom_tx_buffer+tcom_tx_num_bytes, AT_Cmd_Param.IP_Open_Setting.Dest_port,Cmd_Data_length);
    tcom_tx_num_bytes = tcom_tx_num_bytes+Cmd_Data_length;
    tcom_tx_buffer[tcom_tx_num_bytes++] = CHAR_QUOTES;
}
...
void vATProt_Set_TCP_State(uint8_t state)
{
    if(net_Info.tcp_state !=state )
    {
...
uint8_t vATProt_Get_TCP_State(void)
{
    return net_Info.tcp_state;
}
...
{
    if (net_Info.tcp_state >= NET_SIM_CONNECTED)
        return 1;
    else
        return 0;
}
...
//    get_sys_config(config);
    ip_len = DECtoStr((unsigned char *)ip_str, &config[1], 1);
    ip_str[ip_len] = CHAR_STOP;
    ip_len++;
    ip_len += DECtoStr((unsigned char *)&ip_str[ip_len], &config[2], 1);
    ip_str[ip_len] = CHAR_STOP;
    ip_len++;
    ip_len += DECtoStr((unsigned char *)&ip_str[ip_len], &config[3], 1);
    ip_str[ip_len] = CHAR_STOP;
    ip_len++;
    ip_len += DECtoStr((unsigned char *)&ip_str[ip_len], &config[4], 1);
    ip_len++;
    strncpy(AT_Cmd_Param.IP_Open_Setting.Dest_IP,ip_str,ip_len);    
}
#endif
...
static void get_server_port()
{
    ...
    memset(port_str, 0, 6);
//    get_sys_config(config);
    port_len = DECtoStr(port_str, &config[5], 2);
    port_len++;
    strncpy(AT_Cmd_Param.IP_Open_Setting.Dest_port,(char *)port_str,port_len);
}
...
/*        if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_11) == 0)
        {
            DEBUG_PRINT0(DEBUG_MEDIUM, "[2G]:2G module Off\n\r");
            return 0;
        }
        else*/
            return 1;
